...
def create_mock_ants_image(
    shape, spacing=(1.0, 1.0, 1.0), origin=(0.0, 0.0, 0.0)
):
...
class TestApplyTransformsAutoBbox(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_fixed = create_mock_ants_image(
            (20, 20, 20), spacing=(0.5, 0.5, 0.5)
        )
    ...
    @patch(
        "aind_registration_utils.ants.ants.transform_index_to_physical_point"
    )
    def test_identity_transform_no_fixed(
        ...
        # Setup mocks
        mock_idx2phys.side_effect = lambda img, idx: list(
            idx
        )  # index = physical
        ...
        self.assertEqual(
            call_kwargs["spacing"], (1.0, 1.0, 1.0)
        )  # From moving
    ...
    def test_with_fixed_reference(
        ...
        # Continuous indices: x=[4,14], y=[2,12], z=[0,10] → floor/ceil with pad → size=(11,11,11)
    ...
    def test_samples_per_edge_parameter(
        ...
        # Verify more points were generated and passed to transform_index_to_physical_point
...
class TestApplyTransformsAutoBboxEdgeCases(unittest.TestCase):
    ...
    def test_extreme_transform_bounds(
        ...
        self.assertTrue(
            all(isinstance(s, int) for s in image_size)
        )  # Integer sizes
